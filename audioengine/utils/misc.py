...
__all__ = ['recursive_get_size', 'log_init', 'log_critical', 'log_error', 'log_warning', 'log_info', 'log_debug',
           'get_json_file_integrity', 'change_file_extension', 'pad_up_to', 'format_time_diff_for_print', 'timeit',
...
def timeit(method):
    def timed(*args, **kw):
        ...
        time_diff = end_time_ns - start_time_ns
        log_debug('[TIMEIT] {} excecuted in: {:0.4f}s'.format(method.__name__,
                                                              format_time_diff_for_print(time_diff)))
        return result
    return timed

def conditional_decorator(dec, condition):
    """Useful to conditionally @timeit or similar"""
    def decorator(func):
        if not condition:
